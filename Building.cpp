...
void Building::update(Move move){
    ...
    //elevator in service
    bool isService = (!move.isPassMove() && !move.isPickupMove()
        && !move.isSaveMove() && !move.isQuitMove());

    // if elevator is pickup move and is in service
    if (move.isPickupMove() || isService) {
        // sends elevator to current floor
        ...
        if (move.isPickupMove()) {
            ...
            // remove people from floor
            int pickupFloor = elevators[move.getElevatorId()].getCurrentFloor();
            floors[pickupFloor].removePeople(peopleToPickup,
...
int Building::tick(Move move){
    ...
    // for loop to tick all elevators with time
    for (int i = 0; i < NUM_ELEVATORS; i++) {
        elevators[i].tick(time);
    }

    // for loop to tick all floors with time & 
    // keeps track of people who exploded
    int numExploded = 0;
